...
from afkak import (
    create_message, create_gzip_message, create_snappy_message, SimpleProducer,
    KeyedProducer, RoundRobinPartitioner, HashedPartitioner,
    )
...
class TestKafkaProducerIntegration(KafkaIntegrationTestCase):
    ...
    def test_acks_none(self):
        start_offset0 = self.current_offset(self.topic, 0)
        self.current_offset(self.topic, 1)

        producer = SimpleProducer(
            self.client, req_acks=SimpleProducer.ACK_NOT_REQUIRED)
        resp = producer.send_messages(self.topic, self.msg("one"))
    ...
    def assert_produce_request(self, messages, initial_offset, message_ct):
        ...
        self.assertEqual(
            self.current_offset(self.topic, 0), initial_offset + message_ct)
